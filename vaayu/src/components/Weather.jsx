...
function WeatherComponent(props) {
  ...
  useEffect(() => {
    // Fetch weather data and forecast data separately
    fetch(
      `${api.base}weather?q=${defaultLocation}&units=metric&APPID=${api.key}`
    )
      .then((response) => {
        if (!response.ok) {
    ...
    fetch(
      `${api.base}forecast?q=${defaultLocation}&units=metric&cnt=16&APPID=${api.key}`
    )
      .then((response) => {
        if (!response.ok) {
  ...
  }, [defaultLocation]); // Include defaultLocation as a dependency
  ...
  const generateWeatherSummary = (forecastData) => {
    const conditions = forecastData.map((data) =>
    ...
    <div className="weather-app tilt-in-fwd-br">
      <div className="weather-card">
        {weatherData !== null || forecastData !== null ? (
          ...
          <div className="loading-message"><i class="fa-solid fa-map-pin fa-bounce"></i>&nbsp;Fetching Location!&nbsp;<i class="fa-solid fa-gear fa-spin"></i></div>
        )}
      </div>
    </div>
  );
